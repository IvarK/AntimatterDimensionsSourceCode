...
Currency.dimensionBoosts = new class extends NumberCurrency {
  get value() { return player.dimensionBoosts; }
  set value(value) { player.dimensionBoosts = value; }

  get startingValue() {
    if (Player.isInAntimatterChallenge) return 0;
    return Effects.max(
      0,
...
Currency.antimatterGalaxies = new class extends NumberCurrency {
  get value() { return player.galaxies; }
  set value(value) { player.galaxies = value; }

  add(amount) {
    super.add(amount);
    if (Notations.current.name === Notation.cancer.name) player.requirementChecks.permanent.cancerGalaxies += amount;
  }

  get startingValue() {
    if (Player.isInAntimatterChallenge) return 0;
    return InfinityUpgrade.skipResetGalaxy.isBought ? 1 : 0;
  }
}();
