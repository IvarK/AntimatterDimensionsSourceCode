...
(function() {
  function walkSuggestion(suggestion, prefix, output) {
    ...
    for (const s of suggestion.categoryMatches) {
      walkSuggestion(AutomatorGrammar.tokenMap[s], prefix, output);
    }
  }

  CodeMirror.registerHelper("lint", "automato", (contents, _, editor) => {
    const cursor = editor.getDoc().getCursor();
    const parsed = parseAutomatorScript(contents);
    console.log("automato lint");
    const output = [];
    parsed.filter(e => e.$combinedErrors !== undefined).forEach(e => e.$combinedErrors.forEach(m => {
      // We don't spam the user with errors on the current line
      if (e.startLine === cursor.line + 1) return;
      output.push({
        message: m,
        severity: "error",
        from: { line: e.startLine - 1, ch: e.startColumn - 1 },
        to: { line: e.startLine - 1, ch: e.startColumn + e.endOffset - e.startOffset },
      });
    }));
    console.log(output);
    return output;
  ...
  CodeMirror.registerHelper("hint", "anyword", editor => {
    ...
    console.log(__Token);
    console.log(__Token.type);
    let start = __Cursor.ch;
    const end = __Cursor.ch;
    const line = editor.getLine(__Cursor.line);
    while (start && /\w/.test(line.charAt(start - 1)))--start;
    const lineStart = line.slice(0, start);
    const currentPrefix = line.slice(start, end);
    const lineLex = AutomatorGrammar.lexer.tokenize(lineStart);
    console.log(lineLex);
  ...
  CodeMirror.defineSimpleMode("automato", {
    ...
    commandArgs: [
      ...
      { regex: /on|off|infinity|eternity|nowait|respec/ui, token: "variable-2" },
