...
function gainedRealityMachines() {
    let ret = Decimal.pow(1000, player.eternityPoints.plus(gainedEternityPoints()).e / 4000 - 1);
...
function percentToNextGlyphLevel() {
    const ret = gainedGlyphLevel(false).actualLevel;
    let retOffset = 0;
...
function getNewInfReq() {
    ...
    if (!player.infDimensionsUnlocked[1]) return new Decimal("1e1900");
    if (!player.infDimensionsUnlocked[2]) return new Decimal("1e2400");
    if (!player.infDimensionsUnlocked[3]) return new Decimal("1e10500");
    if (!player.infDimensionsUnlocked[4]) return new Decimal("1e30000");
    if (!player.infDimensionsUnlocked[5]) return new Decimal("1e45000");
    if (!player.infDimensionsUnlocked[6]) return new Decimal("1e54000");
    return new Decimal("1e60000");
}
...
function gameLoop(diff, options = {}) {
  ...
  if (Enslaved.isStoringRealTime) {
    ...
    if (autobuyerOnGameLoop) {
      ...
      const autobuyerInterval = BreakInfinityUpgrade.autobuyerSpeed.isBought ? 50 : 100;
  ...
  player.tickThreshold = freeTickspeed.nextShards;
    ...
    if (player.money.gte(Number.MAX_VALUE) && (!player.break || (player.currentChallenge != "" && player.money.gte(player.challengeTarget)))) {
        ...
        if ((player.currentChallenge == "" || player.options.retryChallenge) && (player.bestInfinityTime <= 60000 || player.break)) {} else showTab('emptiness');
    } else document.getElementById("bigcrunch").style.display = 'none';

    const currentIPmin = gainedInfinityPoints().dividedBy(Time.thisInfinity.totalMinutes);
    ...
    const currentEPmin = gainedEternityPoints().dividedBy(player.thisEternity / 60000);
...
function simulateTime(seconds, real, fast) {
    ...
    if (BlackHoles.areUnlocked && !BlackHoles.arePaused) {
      ...
      for (let numberOfTicksRemaining = ticks; numberOfTicksRemaining > 0; numberOfTicksRemaining--) {
        const timeGlyphSpeedup = getGameSpeedupFactor([GameSpeedEffect.TIMEGLYPH]);
        // The black hole is affected by time glyphs, but nothing else.
        const remainingblackHoleSeconds = remainingRealSeconds * timeGlyphSpeedup;
    ...
    let popupString = "While you were away";
...
function showRealityTab(tabName) {
  ...
  ui.view.tabs.reality.subtab = tabName;
    ...
    const tabs = document.getElementsByClassName('realitytab');
    let tab;
    for (let i = 0; i < tabs.length; i++) {
        tab = tabs.item(i);
        if (tab.id === tabName) {
