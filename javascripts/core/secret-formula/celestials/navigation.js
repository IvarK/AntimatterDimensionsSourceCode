...
GameDatabase.celestials.navigation = {
  ...
  "effarig-reality-unlock": {
    ...
    complete: () => (EffarigUnlock.run.isUnlocked 
      ? 1 : Math.clampMin(0.999, Decimal.pLog10(player.celestials.effarig.relicShards) / Math.log10(EffarigUnlock.run.cost))),
    node: {
      completeClass: "c-celestial-nav__effarig",
      incompleteClass: "c-celestial-nav__test-incomplete",
  ...
  "v-unlock-4": {
    ...
    complete: () => {
      if (V.has(V_UNLOCKS.V_ACHIEVEMENT_UNLOCK)) return 1;
      return emphasizeEnd(player.dilation.dilatedTime.pLog10() / GameDatabase.celestials.v.mainUnlock.dilatedTime.log10());
    },
    drawOrder: -1,
    node: {
      incompleteClass: "c-celestial-nav__test-incomplete",
  ...
  "v-unlock-6": {
    ...
    complete: () => {
      if (V.has(V_UNLOCKS.V_ACHIEVEMENT_UNLOCK)) return 1;
      return emphasizeEnd(player.reality.realityMachines.pLog10() / Math.log10(GameDatabase.celestials.v.mainUnlock.rm));
    },
    drawOrder: -1,
    node: {
      incompleteClass: "c-celestial-nav__test-incomplete",
