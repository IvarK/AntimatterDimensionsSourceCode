...
Vue.component("sidebar-ip", {
  ...
  computed: {
    crunchPeaks() {
      return `${format(this.currentIPPM, 2, 0)} IP/min<br>Peaked at ${format(this.peakIPPM, 2, 0)} IP/min`;
    },
  },
  template:
  `<div class="resource">
    <h2 class="o-sidebar-infinity-button">{{ format(ip, 2, 0) }}</h2>
    <div class="resource-information">
      <span class="resource-name">Infinity {{ "Point" | pluralize(ip) }}</span>
      <span class="resource-gain" v-if="showCrunch">+{{format(gained, 2, 0)}}</span>
    </div>
  </div>`
});
