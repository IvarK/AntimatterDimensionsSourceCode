Vue.component('teresa-tab', {
  ...
  methods: {
    update() {
      ...
      this.unlockConst = TERESA_UNLOCKS
  ...
  template:
    `<div class="l-teresa-celestial-tab">
      ...
      <div class="l-teresa-shop">
        <button class="o-teresa-shop-button" @click="buyUnlock(0, 5e6)" :class="{ 'teresa-unlock-bought': unlocks[unlockConst.ADJUSTER] }">Unlock glyph level adjustment.<br>Cost: 5,000,000 Relic Shards</button>
        <button class="o-teresa-shop-button" @click="buyUnlock(1, 2e8)" :class="{ 'teresa-unlock-bought': unlocks[unlockConst.AUTOSACRIFICE] }">Unlock automatic glyph sacrifice.<br>Cost: 200,000,000 Relic Shards</button>
        <button class="o-teresa-shop-button" @click="buyUnlock(2, 5e8)" :class="{ 'teresa-unlock-bought': unlocks[unlockConst.AUTOPICKER] }">Unlock automatic glyph picker.<br>Cost: 500,000,000 Relic Shards</button>
      </div>
      <button class="o-teresa-shop-button" @click="buyUnlock(3, 1e9)" :class="{ 'teresa-unlock-bought': unlocks[unlockConst.RUN] }">Unlock Teresa's reality.<br>Cost: 1,000,000,000 Relic Shards</button>
      <div class="l-teresa-glyph-settings">
        <div v-if="unlocks[unlockConst.AUTOSACRIFICE]">
        ...
        <div v-if="unlocks[unlockConst.ADJUSTER]">
          <div>
        ...
        <div v-if="unlocks[unlockConst.AUTOPICKER]">Glyph effect weight settings here</div>
      </div>
      <div v-if="unlocks[unlockConst.RUN]"><button class="o-teresa-shop-button teresa-run-button" @click="startRun()">Start a new reality, all production and gamespeed is severely lowered and you can only use one glyph, infinity and time dimensions reduce the production penalty. IP multipliers are disabled. You will gain unlocks at Infinity, Eternity and Reality.</button>
        <div v-if="unlocks[unlockConst.INFINITY_COMPLETE]">Infinity: IP multipliers have partial effectiveness in Teresa Reality, infinitied stat raises the replicanti cap and increases your max RG.</div>
        <div v-if="unlocks[unlockConst.ETERNITY_COMPLETE]">Eternity: You can now use 5 glyphs and production penalty is reduced in Teresa Reality; eternitied stat generates infinitied stat, unlocks The Enslaved Ones.</div>
        <div v-if="unlocks[unlockConst.REALITY_COMPLETE]">Reality: (Reward not implemented yet)</div>
      </div>
    </div>`
});