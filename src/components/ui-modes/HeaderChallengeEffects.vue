  ...
  methods: {
    ...
    updateChallengePower() {
      ...
      if (isChallengePowerVisible) {
        const powerArray = [];
        if (isC2Running) powerArray.push(`Production: ${formatPercents(player.cValues.c2Pow, 2, 2)}`);
        if (isC3Running) powerArray.push(`First dimension: ${formatX(player.cValues.c3Pow, 3, 4)}`);
