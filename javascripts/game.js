...
export function gainedEternities() {
  return new Decimal(RealityUpgrade(3).effectOrDefault(1))
    .times(getAdjustedGlyphEffect("timeetermult"));
}

export function gainedBankedInfinities() {
  return new Decimal(Achievement(131).effectOrDefault(0)).plus(TimeStudy(191).effectOrDefault(0));
}
...
export function gameLoop(passDiff, options = {}) {
  ...
  if (player.dilation.active && Ra.has(RA_UNLOCKS.AUTO_TP)) Reset.exitDilation.gain();
