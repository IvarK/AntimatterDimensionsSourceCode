...
function formatValue(x, places) {
  if (x instanceof Decimal) {
    ...
    return (x.m.toFixed(places) + "e" + x.e);
  }
...
function prestige(id) {
  ...
  const prestiges = document.getElementsByClassName("prestigebtn");
...
function load() {
  const save = JSON.parse(localStorage.getItem("funsave"));
  if (save) game = save;
  ...
  for (var i = 1; i < game.costs.length - 5; i++) {
    const pbtn = document.createElement("button");
    const otherbtn = document.getElementById(i);
...
setInterval(() => {
  ...
  for (let i = 2; i <= game.costs.length; i++) {
